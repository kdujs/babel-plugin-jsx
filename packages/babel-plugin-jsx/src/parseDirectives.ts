...
const parseDirectives = (params: {
  ...
  const isKModels = directiveName === 'models';
  const isKModel = directiveName === 'model';
  if (isKModel && !t.isJSXExpressionContainer(path.get('value'))) {
    throw new Error('You have to use JSX Expression inside your k-model');
  }

  if (isKModels && !isComponent) {
    throw new Error('k-models can only use in custom components');
  }

  const shouldResolve = !['html', 'text', 'model', 'models'].includes(directiveName)
    || (isKModel && !isComponent);

  let modifiers = directiveModifiers;

  if (t.isArrayExpression(value)) {
    const elementsList = isKModels ? value.elements! : [value];

    elementsList.forEach((element) => {
      if (isKModels && !t.isArrayExpression(element)) {
        throw new Error('You should pass a Two-dimensional Arrays to k-models');
      }
      ...
      if (second && !t.isArrayExpression(second) && !t.isSpreadElement(second)) {
        ...
        // work as k-model={[value]} or k-models={[[value]]}
        args.push(t.nullLiteral());
      }
      modifiersSet.push(new Set(modifiers));
      vals.push(first as t.Expression);
    });
  } else if (isKModel && !shouldResolve) {
    // work as k-model={value}
    args.push(t.nullLiteral());
...
const resolveDirective = (
  ...
  if (directiveName === 'show') {
    return createIdentifier(state, 'kShow');
  }
  if (directiveName === 'model') {
    ...
    switch ((tag as t.StringLiteral).value) {
      case 'select':
        modelToUse = createIdentifier(state, 'kModelSelect');
        break;
      case 'textarea':
        modelToUse = createIdentifier(state, 'kModelText');
        break;
      default:
        if (t.isStringLiteral(type) || !type) {
          switch ((type as t.StringLiteral)?.value) {
            case 'checkbox':
              modelToUse = createIdentifier(state, 'kModelCheckbox');
              break;
            case 'radio':
              modelToUse = createIdentifier(state, 'kModelRadio');
              break;
            default:
              modelToUse = createIdentifier(state, 'kModelText');
          }
        } else {
          modelToUse = createIdentifier(state, 'kModelDynamic');
        }
    }
    return modelToUse;
  }
