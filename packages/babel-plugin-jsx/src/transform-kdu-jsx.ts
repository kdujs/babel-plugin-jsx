        ...
        if (state.opts.transformOn && (name === 'on' || name === 'nativeOn')) {
          if (!state.get('transformOn')) {
            state.set('transformOn', addDefault(
              path,
              '@kdujs/babel-helper-kdu-transform-on',
...
const getChildren = (
  ...
  .map((path) => {
    if (path.isJSXText()) {
      ...
      if (transformedText) {
        return t.callExpression(createIdentifier(state, 'createTextKNode'), [transformedText]);
      }
...
const transformJSXElement = (
  ...
  if (children.length > 1 || slots) {
    /*
      <A k-slots={slots}>{a}{b}</A>
        ---> {{ default: () => [a, b], ...slots }}
        ---> {[a, b]}
    */
    KNodeChild = isComponent
      ? children.length
        ? t.objectExpression([
    ...
    if (t.isIdentifier(child) && isComponent) {
      KNodeChild = enableObjectSlots ? t.conditionalExpression(
        t.callExpression(state.get('@kdujs/babel-plugin-jsx/runtimeIsSlot')(), [child]),
        child,
        objectExpression,
      ) : objectExpression;
    } else if (
      ...
      if (enableObjectSlots) {
        ...
        const condition = t.callExpression(
          state.get('@kdujs/babel-plugin-jsx/runtimeIsSlot')(),
          [assignment],
        );
        KNodeChild = t.conditionalExpression(
          condition,
          slotId,
          alternate,
        );
      } else {
        KNodeChild = objectExpression;
      }
    } else if (t.isFunctionExpression(child) || t.isArrowFunctionExpression(child)) {
      KNodeChild = t.objectExpression([
        t.objectProperty(
          t.identifier('default'),
          child,
        ),
      ...
      KNodeChild = isComponent ? t.objectExpression([
        t.objectProperty(
          t.identifier('default'),
  ...
  const createKNode = t.callExpression(createIdentifier(state, 'createKNode'), [
    tag,
    props,
    KNodeChild || t.nullLiteral(),
